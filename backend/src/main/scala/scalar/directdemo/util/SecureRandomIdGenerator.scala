...
case class SecureRandomIdGenerator(sizeInBytes: Int):
  ...
  private val cachedRand: SecureRandom =
    val r = SecureRandom.getInstance(
      if (scala.util.Properties.isWin) "Windows-PRNG" else "NativePRNGNonBlocking"
    )
  ...
  private def toHexString(byteArray: Array[Byte]) =
    byteArray.map(b => String.format("%02x", b)).mkString
