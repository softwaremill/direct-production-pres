...
/** Helper object for defining HTTP endpoints. Import as `Http.*` to gain access to Tapir's API and
  * customizations.
  */
object Http extends Tapir with TapirJsonJsoniter with Logging:
  ...
  /** Description of the output, that is used to represent an error that occurred during endpoint
    * invocation.
    */
  ...
  /** Base endpoint description for non-secured endpoints. Specifies that errors are always returned
    * as JSON values corresponding to the [[Error_OUT]] class, translated from a [[Fail]] instance.
    */
  ...
  /** Base endpoint description for secured endpoints. Specifies that errors are always returned as
    * JSON values corresponding to the [[Error_OUT]] class, translated from a [[Fail]] instance, and
    * that authentication is read from the `Authorization: Bearer` header.
    */
