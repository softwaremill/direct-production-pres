...
import scalar.directdemo.email.sender.DummyEmailSender
import scalar.directdemo.passwordreset.PasswordResetApi.{ForgotPassword_OUT, PasswordReset_OUT}
import scalar.directdemo.test.*
...
class PasswordResetApiTest extends BaseTest with Eventually with TestDependencies with TestSupport:
  ...
  def codeSentToEmail(email: String): String = {
    ...
    val emailData = DummyEmailSender
      .findSentEmail(email, "SoftwareMill Directdemo password reset")
  ...
  def codeWasNotSentToEmail(email: String): Unit =
    ...
    val maybeEmail = DummyEmailSender.findSentEmail(email, "SoftwareMill Directdemo password reset")
    maybeEmail match {
      case Some(emailData) =>
